...
def on_system_event(hwnd, msg, wparam, lparam):
    if msg == win32con.WM_POWERBROADCAST:
        if wparam == win32con.PBT_APMSUSPEND:
            insert_Xtra_info(False) #indicating that system is going to sleep
        elif wparam == win32con.PBT_APMQUERYSUSPEND:
            insert_Xtra_info(False) #indicating that system is going to hybernate
        elif wparam == win32con.PBT_APMRESUMEAUTOMATIC:
            insert_Xtra_info(True) # indicating that system is waking up
